  ...
  websocket: {
    ...
    initializeConnection: () => {
      ...
      newSocket.on("transport-sync", (command) => {
        ...
        // Spectators control their own playback locally; ignore remote transport
        if (get().websocket.isSpectator) return;
    ...
    syncFromServer: (roomState) => {
      ...
      if (roomState.trackEffects) {
        const { effects } = get();
        Object.keys(roomState.trackEffects).forEach((trackId) => {
          effects.syncTrackEffectState(trackId, roomState.trackEffects[trackId]);
        });
      }
      if (roomState.users) {
