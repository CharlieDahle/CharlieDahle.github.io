...
function Photos() {
    ...
    const tick = (timestamp) => {
      ...
      for (const laneNum of [1, 2]) {
        ...
        for (let i = 0; i < lane.length; i++) {
          ...
          car.wheelRotation -=
            (car.currentSpeed / WHEEL_RADIUS) * dt * (180 / Math.PI) * 0.4;
          ...
          if (frontEl)
            frontEl.style.transform = `rotate(${car.wheelRotation}deg)`;
          if (rearEl)
            rearEl.style.transform = `rotate(${car.wheelRotation}deg)`;

          if (laneNum === 1 && car.x > vw + car.width + 50)
  ...
  const handleTruckClick = useCallback((id) => {
    ...
    setCarList((prev) =>
      prev.map((c) => (c.id === id ? { ...c, stopped: car.stopped } : c)),
    );
  }, []);
  ...
  return (
    <div className="photos-root">
      <header className="photos-header">
        <a href="https://charliedahle.me" className="photos-header-back">
          ← charliedahle.me
        </a>
        <button
        ...
        >
          ...
          >
            <path
              d="M7 1.5L5.5 4H2C1.4 4 1 4.4 1 5V15C1 15.6 1.4 16 2 16H18C18.6 16 19 15.6 19 15V5C19 4.4 18.6 4 18 4H14.5L13 1.5H7Z"
              stroke="currentColor"
              strokeWidth="1.5"
      ...
      {uploadStatus === "success" && (
        <div className="upload-toast">Photo uploaded!</div>
      )}
      {uploadStatus === "error" && (
        <div className="upload-toast upload-toast--error">
        ...
        {carList.map((car) => {
          ...
          return (
            <React.Fragment key={car.id}>
              ...
              <div
                ref={(el) => {
                  if (el) containerRefs.current[car.id] = el;
                }}
              ...
              >
                ...
                <img
                  ref={(el) => {
                    if (el) wheelFrontRefs.current[car.id] = el;
                  }}
                  src={wheelSvg}
                  className="wheel"
                  style={{
                ...
                <img
                  ref={(el) => {
                    if (el) wheelRearRefs.current[car.id] = el;
                  }}
                  src={wheelSvg}
                  className="wheel"
                  style={{
              ...
              <div
                ref={(el) => {
                  if (el) photoContainerRefs.current[car.id] = el;
                }}
                className={`truck-photo-outer lane-${car.lane}`}
                style={{
                  transform: `translateX(${car.initialPhotoX}px) translateY(${PHOTO_TRANSLATE_Y}px)`,
                }}
                onClick={() => handleTruckClick(car.id)}
              >
                <div
                ...
                >
                  ...
                  {car.stopped && (
                    <div className="photo-expand-hint">click to expand</div>
                  )}
                </div>
              </div>
            </React.Fragment>
