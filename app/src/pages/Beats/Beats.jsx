...
function Beats() {
  ...
  const [beatRequests, setBeatRequests] = useState({}); // { [beatId]: [requests] }
  const [requestsModalBeat, setRequestsModalBeat] = useState(null);
  const [requestsRoleDropdown, setRequestsRoleDropdown] = useState(null);
  ...
  // Fetch pending requests for all owned beats
  useEffect(() => {
    if (!isAuthenticated || !userBeats.length) return;
    const getAuthHeaders = useAppStore.getState().auth.getAuthHeaders;
    const fetchRequests = async () => {
      const results = {};
      await Promise.all(
        userBeats.map(async (beat) => {
          const beatKey = beat.room_id || beat.id;
          try {
            const res = await fetch(`https://api.charliedahle.me/api/beats/${beatKey}/requests`, {
              headers: getAuthHeaders(),
            });
            if (res.ok) {
              const data = await res.json();
              if (data.requests.length > 0) results[beat.id] = data.requests;
            }
          } catch (err) {
            console.error("[Requests] Error for", beatKey, ":", err);
          }
        })
      );
      setBeatRequests(results);
    };
    fetchRequests();
  }, [userBeats, isAuthenticated]);

  const handleRespondToRequest = async (beat, requestId, approve, role = "temporary") => {
    const getAuthHeaders = useAppStore.getState().auth.getAuthHeaders;
    setRequestsRoleDropdown(null);
    try {
      await fetch(`https://api.charliedahle.me/api/beats/${beat.room_id || beat.id}/requests/${requestId}`, {
        method: "POST",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({ approve, role }),
      });
      setBeatRequests((prev) => {
        const updated = (prev[beat.id] || []).filter((r) => r.id !== requestId);
        return { ...prev, [beat.id]: updated.length ? updated : undefined };
      });
    } catch (err) {
  ...
  const renderOwners = (beat) => {
    const names = activeTab === "my-beats" ? (beat.co_owners || []) : (beat.owners || []);
    const prefix = activeTab === "my-beats" ? "You + " : "by ";
    if (!names.length) return null;
    const shown = names.slice(0, 2).join(", ");
    const extra = names.slice(2);
    return (
      <span className="beat-owners">
        {prefix}{shown}
        {extra.length > 0 && (
          <span className="beat-owners-more" data-tooltip={extra.join(", ")}>
            {" "}+{extra.length} more
          </span>
        )}
      </span>
  ...
  const handleStartEditName = (beat, e) => {
    e.stopPropagation();
    setEditingNameId(beat.id);
    setEditingNameValue(beat.name);
  };

  const handleSaveName = async (beat) => {
    const trimmed = editingNameValue.trim();
    setEditingNameId(null);
    if (!trimmed || trimmed === beat.name) return;
    const getAuthHeaders = useAppStore.getState().auth.getAuthHeaders;
    try {
      const res = await fetch(`https://api.charliedahle.me/api/beats/${beat.room_id || beat.id}/name`, {
        method: "PATCH",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({ name: trimmed }),
      });
      if (res.ok) fetchUserBeats();
    } catch (err) {
      console.error("Failed to rename beat:", err);
    }
  };
  ...
  return (
    <div className="beats-page">
          ...
          <div className="beats-container">
            ...
            <div className="content-header">
              <div className="search-section">
                ...
                />
                {activeTab === "public" && (
                  <button className="refresh-btn" onClick={fetchPublicBeats} title="Refresh public beats">
                    <RefreshCw size={15} />
                    <span>Refresh</span>
                  </button>
                )}
                  ...
                  return (
                    ...
                    >
                      <div className="beat-card-header">
                        {activeTab === "my-beats" && editingNameId === beat.id ? (
                          <input
                            className="beat-name-input"
                            value={editingNameValue}
                            onChange={(e) => setEditingNameValue(e.target.value)}
                            onBlur={() => handleSaveName(beat)}
                            onKeyDown={(e) => {
                              if (e.key === "Enter") handleSaveName(beat);
                          ...
                          <h3
                            className={`beat-name ${activeTab === "my-beats" ? "beat-name--editable" : ""}`}
                            onClick={activeTab === "my-beats" ? (e) => handleStartEditName(beat, e) : undefined}
                          >
                            {beat.name}
                            {isCurrentlyLoaded && (
                      ...
                      <div className="beat-card-footer">
                        ...
                        {activeTab === "my-beats" && (
                          <>
                            {/* Edit requests badge */}
                            {beatRequests[beat.id]?.length > 0 && (
                              <button
                                className="requests-badge-btn"
                                onClick={() => setRequestsModalBeat(beat)}
                                title="Pending edit requests"
                              >
                                <Inbox size={15} />
                                <span>{beatRequests[beat.id].length}</span>
                              </button>
                            )}
                            <button
      ...
      {/* Edit Requests Modal */}
      {requestsModalBeat && (
        <div className="requests-modal-overlay" onClick={() => { setRequestsModalBeat(null); setRequestsRoleDropdown(null); }}>
          <div className="requests-modal" onClick={(e) => e.stopPropagation()}>
            <div className="requests-modal-header">
              <div>
                <h3 className="requests-modal-title">Edit Requests</h3>
                <span className="requests-modal-beat-name">"{requestsModalBeat.name}"</span>
              </div>
              <button className="requests-modal-close" onClick={() => { setRequestsModalBeat(null); setRequestsRoleDropdown(null); }}>
                <X size={18} />
              </button>
            </div>
            <div className="requests-modal-body">
              {!beatRequests[requestsModalBeat.id]?.length ? (
                <p className="requests-modal-empty">No pending requests</p>
              ) : (
                beatRequests[requestsModalBeat.id].map((req) => (
                  <div key={req.id} className="requests-modal-row">
                    <div className="requests-modal-left">
                      <span className="requests-modal-username">{req.username || "Guest"}</span>
                      <span className="requests-modal-time">
                        <Clock size={11} />
                        {getTimeAgo(req.requested_at)}
                      </span>
                    </div>
                    <div className="requests-modal-actions">
                      <div className="req-accept-btn-group">
                        <button
                          className="req-accept-main"
                          onClick={() => handleRespondToRequest(requestsModalBeat, req.id, true, "temporary")}
                        >
                          <Check size={14} />
                          Accept
                        </button>
                        <button
                          className="req-accept-dropdown-toggle"
                          onClick={() => setRequestsRoleDropdown(requestsRoleDropdown === req.id ? null : req.id)}
                        >
                          <ChevronDown size={14} />
                        </button>
                        {requestsRoleDropdown === req.id && (
                          <div className="req-accept-dropdown">
                            <button
                              className="req-accept-dropdown-item"
                              onClick={() => handleRespondToRequest(requestsModalBeat, req.id, true, "temporary")}
                            >
                              <span className="req-dropdown-title">Temporary Editor</span>
                              <span className="req-dropdown-desc">Can edit during this session only</span>
                            </button>
                            <button
                              className="req-accept-dropdown-item"
                              onClick={() => handleRespondToRequest(requestsModalBeat, req.id, true, "collaborator")}
                            >
                              <span className="req-dropdown-title">Collaborator</span>
                              <span className="req-dropdown-desc">Can edit anytime, no ownership</span>
                            </button>
                            <button
                              className="req-accept-dropdown-item"
                              onClick={() => handleRespondToRequest(requestsModalBeat, req.id, true, "co-owner")}
                            >
                              <span className="req-dropdown-title">Co-Owner</span>
                              <span className="req-dropdown-desc">Full permissions including delete</span>
                            </button>
                          </div>
                        )}
                      </div>
                      <button
                        className="req-deny"
                        onClick={() => handleRespondToRequest(requestsModalBeat, req.id, false)}
                      >
                        <X size={14} />
                        Deny
                      </button>
